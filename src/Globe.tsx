...
export default function Globe() {
  ...
  return (
    <section className="globe-sticky" style={{ position: 'relative', width: '100%', overflow: 'hidden', ...(isMobile ? { height: 'auto' } : null) }}>
      {!isMobile && <canvas ref={canvasRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }} />}

      {/* HERO */}
      <div className="hero-panel" style={{ ...(isMobile ? { position: 'relative' } : { position: 'absolute', top: 0, left: 0, height: '100%' }), width: 'min(560px,50%)', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 0 0 clamp(24px,5vw,72px)', zIndex: 10, pointerEvents: 'none' }}>
